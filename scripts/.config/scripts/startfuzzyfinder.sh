...
if [ "$file_info" = "directory" ]; then
    ...
    cd "$file_directory"
    filename="$(basename "$pathtocheck")"
    lower_filename="$(echo "$filename" | tr '[:upper:]' '[:lower:]')"
    # Check if it's a PDF (case-insensitive, POSIX compatible)
    if echo "$lower_filename" | grep -q '\.pdf$'; then
        # nohup evince "$filename" >/dev/null 2>&1 &
        nohup evince "$filename" >/dev/null 2>&1 &
        disown
    else
        "$EDITOR" "$filename"
