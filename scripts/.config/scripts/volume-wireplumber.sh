...
print_status() {
    # Get volume status in one call
    local status vol icon tooltip

    status="$(wpctl get-volume @DEFAULT_AUDIO_SINK@ 2>/dev/null || echo "Volume: 0.00")"

    # Parse volume percentage
    vol="$(echo "$status" | awk '{printf("%d\n", $2*100)}')"

    # Check mute status and select icon
    ...
    printf '{"text":"%s","tooltip":"%s","class":"volume","percentage":%d}\n' "$icon" "$tooltip" "$vol"
}

# Print immediately on startup
print_status

# Subscribe to PulseAudio/PipeWire events
# Filter to only react to relevant audio changes
